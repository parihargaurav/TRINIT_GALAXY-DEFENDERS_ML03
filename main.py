...
model = pickle.load(open('C:\Streamlit\Scripts\model.pkl', 'rb'))
# dataset file
df = pd.read_csv("C:\\Users\\Atul Parihar\\Downloads\\Crop_recommendation.csv")
...
def scatterPlotDrawer(x, y):
    ...
    sns.scatterplot(data=df, x=x, y=y, hue="label", size="label",
                    palette="deep", sizes=(20, 200), legend="full")
...
def main():
    ...
    if select_type == "Predict Your Crop":
        ...
        if st.button("Predict Price"):
            ...
            k = {}
            l = {}
            for i in data['commodity'].values:
